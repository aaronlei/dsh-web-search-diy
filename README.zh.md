...
该文件**按协议模式分桶**（`{ "version": 2, "mode": …, "modes": { <模式>: { … } } }`）：
每个模式各自保存自己的端点、模型、凭据引用、输出预算与搜索选项，因此进入某模式时
恢复的是它自己的设置——刷新页面后依然如此——既不会把上一个模式的取值带过去，也不会
被规范默认值覆盖。尚无桶的模式会从该模式的官方值起步。0.3.x 写出的扁平文件在读取时
被投影到它当时选中的模式，下一次保存即改写为分桶结构；读取过程不写盘。
...
切换协议时，端点、模型与凭据引用会一并跟随：属于“正在离开的那个服务商家族”的
值会被替换，不属于任何已知家族的自定义值（网关令牌等）原样保留。由于同一家族可能
有多个都算官方的取值，卡片还会记住每个家族本次用过的那个——用
`ZAI_CODING_CN_API_KEY` 的智谱部署切回来时拿回的是它，而不是自己从未配置过的
规范值 `ZHIPU_API_KEY`。
