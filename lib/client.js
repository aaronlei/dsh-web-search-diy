	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			baseURLHint: "切换协议时自动填入官方地址；仍可改为任意自定义地址。",
			apiKeyEnv: "凭据引用 apiKeyEnv",
			apiKeyEnvHint: "留空则使用当前协议模式的默认引用（OpenAI 兼容：QWEN_TOKEN_PLAN_CN_API_KEY；智谱：ZHIPU_API_KEY）。",
			model: "模型 model",
			modelHint: "留空使用默认；OpenAI 兼容模式为 deepseek-v4-flash-0731，智谱问答增强为 glm-5.3-flash（配思考强度 low）。免费档 glm-4.7-flash 经常限流不可用，不建议。",
		...
		const en = {
			...
			baseURLHint: "Auto-filled with the official endpoint on protocol switch; still yours to override.",
			...
			model: "Model",
			modelHint: "Leave blank for the default: deepseek-v4-flash-0731 in OpenAI-compatible mode, glm-5.3-flash (with thinking effort low) in Zhipu chat mode. The free-tier glm-4.7-flash is frequently rate-limited and not recommended.",
		...
		*/
		...
		const DRAFT_KEYS = ["mode", "baseURL", "apiKeyEnv", "model", "maxOutputTokens", "searchEngine", "count", "searchRecencyFilter", "contentSize", "reasoningEffort", "responsesReasoningEffort", "searchDomainFilter", "searchIntent", "searchPrompt"];
		/**
				...
				children: [
							...
							loaded ? [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ChoiceField, {
									...
									onEdit: (value) => {
										setDraft((current) => ({ ...current, mode: value, baseURL: followModeEndpoint(current.baseURL, value) }));
									}
								}, "mode"),
