	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			baseURLHint: "切换协议时自动填入官方地址；仍可改为任意自定义地址。DeepSeek 官方模式留空时还会回退到环境变量 DEEPSEEK_SEARCH_BASE_URL。",
			...
			maxOutputTokensHint: "单次搜索回合（推理 + 工具调用 + 回答）的输出上限；留空使用该模式默认值（DeepSeek 官方 4096，其余模式 1024）。",
			apiVersion: "协议版本 apiVersion",
			apiVersionHint: "仅 DeepSeek 官方模式生效；作为 anthropic-version 请求头发送，官方默认 2023-06-01。",
			maxUses: "搜索次数上限 maxUses",
			maxUsesHint: "仅 DeepSeek 官方模式生效；单次请求内 web_search 服务端工具最多可调用几次，官方默认 5。",
		...
		const en = {
			...
			baseURLHint: "Auto-filled with the official endpoint on protocol switch; still yours to override. In DeepSeek official mode a blank endpoint also falls back to the DEEPSEEK_SEARCH_BASE_URL environment variable.",
			...
			maxOutputTokensHint: "Output cap for one search turn (reasoning + tool use + answer). Leave blank for the mode default: 4096 in DeepSeek official mode, 1024 elsewhere.",
			apiVersion: "Protocol version (apiVersion)",
			apiVersionHint: "DeepSeek official mode only; sent as the anthropic-version header, default 2023-06-01.",
			maxUses: "Search uses per request (maxUses)",
			maxUsesHint: "DeepSeek official mode only; how many times the web_search server tool may run in one request, default 5.",
		...
		*/
		...
		function toSnapshot(data) {
			return {
				...
				anthropicThinking: data.anthropicThinking ?? "default",
				...
				responsesReasoningEffort: data.responsesReasoningEffort ?? "",
		...
		*/
		...
		const DRAFT_KEYS = ["mode", "baseURL", "apiKeyEnv", "model", "maxOutputTokens", "apiVersion", "maxUses", "anthropicThinking", "searchEngine", "count", "searchRecencyFilter", "contentSize", "reasoningEffort", "responsesReasoningEffort", "searchDomainFilter", "searchIntent", "searchPrompt"];
		/**
		* Each mode's official endpoint. Switching modes fills this value into the
		* endpoint field — the addresses are settled facts, not something the user
		* should have to look up — while a customized endpoint survives the switch.
		*/
		const OFFICIAL_ENDPOINTS = {
			...
			"zhipu-web-search": "https://open.bigmodel.cn/api/paas/v4",
			"zhipu-chat-search": "https://open.bigmodel.cn/api/paas/v4"
		};
		/**
		...
		*/
		const MODE_MAX_OUTPUT_TOKENS = {
			...
			"zhipu-web-search": 1024,
			"zhipu-chat-search": 1024
		};
		/**
		* Each mode's official model and credential reference, alongside its
		* endpoint. A stale GLM model name or a Zhipu key reference must not ride
		* along into another protocol, and the schema/UI file can hold either, so
		* switching modes replaces a stock value with the new mode's own.
		*/
		const OFFICIAL_MODELS = {
			responses: "deepseek-v4-flash-0731",
			"anthropic-messages": "deepseek-flash",
			"zhipu-web-search": "glm-5.3-flash",
			"zhipu-chat-search": "glm-5.3-flash"
		};
		/**
		* Known credential references per protocol family, most-canonical first.
		*
		* A deployment usually names its Zhipu credential after the provider it
		* configured there (`zai-coding-cn` → `ZAI_CODING_CN_API_KEY`) rather than
		* after this plugin's own default, so recognizing the whole family — not
		* just one string — is what makes a protocol switch swap the reference
		* instead of carrying a Zhipu key onto the DeepSeek endpoint.
		*/
		const OFFICIAL_KEY_REFS = {
			responses: ["QWEN_TOKEN_PLAN_CN_API_KEY"],
			"anthropic-messages": ["DEEPSEEK_API_KEY"],
			"zhipu-web-search": ["ZHIPU_API_KEY", "ZAI_CODING_CN_API_KEY"],
			"zhipu-chat-search": ["ZHIPU_API_KEY", "ZAI_CODING_CN_API_KEY"]
		};
		/**
		* Endpoint the draft carries after a mode switch: the new mode's official
		* endpoint, unless the user customized the field (any value that is not
		* blank and not some mode's official endpoint is theirs to keep).
		*/
		function followModeEndpoint(currentURL, nextMode) {
			const isStock = currentURL === "" || Object.values(OFFICIAL_ENDPOINTS).includes(currentURL);
			return isStock ? OFFICIAL_ENDPOINTS[nextMode] : currentURL;
		}
		/**
		* The same rule for a text-valued table (model, credential reference).
		* Comparison ignores case: model ids reach the file through several hands
		* (`GLM-5.3-Flash` and `glm-5.3-flash` are the same model), and a stock
		* value must be recognized as stock regardless of how it was cased.
		*
		* @param table - the per-mode official values.
		* @param currentValue - the draft's current value.
		* @param nextMode - the mode being switched to.
		* @returns the value the draft should carry.
		*/
		function followModeValue(table, currentValue, nextMode) {
			const stock = Object.values(table).map((value) => value.toLowerCase());
			const isStock = currentValue === "" || stock.includes(currentValue.toLowerCase());
			return isStock ? table[nextMode] : currentValue;
		}
		/**
		* The credential reference the draft carries after a mode switch.
		*
		* A blank field takes the target's canonical reference. A reference that
		* belongs to some other protocol family is a stock value for the protocol
		* being left, so it follows too — otherwise switching Zhipu → DeepSeek
		* would send a Zhipu key to the DeepSeek endpoint. A reference outside every
		* known family is the deployment's own (a gateway token, a custom name) and
		* is never rewritten.
		*
		* @param currentValue - the draft's current `apiKeyEnv`.
		* @param nextMode - the mode being switched to.
		* @returns the reference the draft should carry.
		*/
		function followModeKeyRef(currentValue, nextMode) {
			const target = OFFICIAL_KEY_REFS[nextMode];
			if (currentValue === "") return target[0];
			const currentFamily = Object.values(OFFICIAL_KEY_REFS).find((family) => family.some((ref) => ref.toLowerCase() === currentValue.toLowerCase()));
			return currentFamily === void 0 ? currentValue : target[0];
		}
				...
				children: [
							...
							loaded ? [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ChoiceField, {
									...
									value: draft.mode,
									options: [
										{ value: "anthropic-messages", label: t("modeAnthropic") },
									...
									onEdit: (value) => {
										setDraft((current) => ({
											...current,
											mode: value,
											baseURL: followModeEndpoint(current.baseURL, value),
											model: followModeValue(OFFICIAL_MODELS, current.model, value),
											apiKeyEnv: followModeKeyRef(current.apiKeyEnv, value)
										}));
									}
								}, "mode"),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ValueField, {
									...
									label: t("baseURL"),
									hint: t("baseURLHint"),
									placeholder: OFFICIAL_ENDPOINTS[draft.mode],
									text: draft.baseURL,
									onEdit: (text) => {
								...
								draft.mode === "anthropic-messages" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ValueField, {
									...
									onEdit: (text) => {
										setDraft((current) => ({ ...current, apiVersion: text }));
									}
								}, "apiVersion") : null,
								draft.mode === "anthropic-messages" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ValueField, {
									...
									text: draft.maxUses,
									onEdit: (text) => {
										setDraft((current) => ({ ...current, maxUses: text }));
									}
								}, "maxUses") : null,
								draft.mode === "anthropic-messages" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ChoiceField, {
									...
									options: [
										{ value: "default", label: t("effortDefault") },
										{ value: "disabled", label: t("thinkingOff") }
									],
									onEdit: (value) => {
										setDraft((current) => ({ ...current, anthropicThinking: value }));
									}
								...
								draft.mode === "responses" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ChoiceField, {
									...
									onEdit: (value) => {
										setDraft((current) => ({ ...current, responsesReasoningEffort: value }));
									}
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				ctx.slots.inject("plugins.row.config", () => ctx.slots.register({
					...
					locale: NS
				}, (props) => props.view === "summary" ? props.t("description") : (0, react_jsx_runtime.jsx)(ConfigPage, {
					t: props.t
				})));
