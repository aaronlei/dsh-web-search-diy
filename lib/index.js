		...
		handler: async (req, res) => {
			...
			if (req.method === "POST") {
				...
				if (typeof body.apiKey === "string" && body.apiKey.length > 0) {
					...
					const mode = patch.mode ?? current.mode ?? DEFAULT_MODE;
					const storedRef = cleared.includes("apiKeyEnv") ? void 0 : current.apiKeyEnv;
				...
				const config = saveFileConfig(patch, cleared);
				const keyConfigured = await hasCredential(ctx, apiKeyEnvCandidates(config.apiKeyEnv, config.mode ?? DEFAULT_MODE));
...
/**
 * Merge and persist the UI-managed configuration file. Cleared keys are deleted
 * rather than written blank, so the file only ever holds real choices. Returns
 * the merged config (used for the keyConfigured echo).
 *
 * @param partial - the keys to merge in.
 * @param cleared - the keys to drop so their mode default applies again.
 * @returns the persisted configuration.
 */
function saveFileConfig(partial, cleared = []) {
